...
class LedgerEntryServiceIntegrationTest {
    ...
    void recordsEntryAndPersistsAllFields() {
        RecordLedgerEntryResult result = ledgerEntryService.recordEntry(command("payment:payment-1:completed"));
    ...
    void returnsPersistedEntryWhenSameIdempotencyKeyIsRecordedAgain() {
        RecordLedgerEntryResult first = ledgerEntryService.recordEntry(command("payment:payment-1:completed"));

        RecordLedgerEntryResult second = ledgerEntryService.recordEntry(command("payment:payment-1:completed"));
    ...
    void recordsDifferentTransactionsWithDifferentIdempotencyKeys() {
        RecordLedgerEntryResult first = ledgerEntryService.recordEntry(command("payment:payment-1:completed"));
        RecordLedgerEntryResult second = ledgerEntryService.recordEntry(command("payment:payment-2:completed"));
